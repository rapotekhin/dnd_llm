...
В каталоге `tests/` лежат модульные и интеграционные проверки для кода под `game/core` (утилиты, загрузчики JSONL, торговля, сохранения, билдеры персонажа и левелапа). Циклы exploration и social в тестах гоняются с **подставными агентами** — без реальных вызовов OpenRouter.
